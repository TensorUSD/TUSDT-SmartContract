...
after 1 hour:       debt = 800 × (1 + 0.02/8760) = 800.001826484 TAO (interest 0.001826484)
after 30 days (720 h):  debt = 800 × 1.0016451855... = 801.316148460 TAO (interest 1.316148460)
                    exchange rate = 1.000526165581675849; Alice's 1,000 lTokens
                    are worth 1,000.526165581 TAO → repaying 801.316148460 TAO
                    clears Charlie's position exactly
after 1 year (8,760 h):  debt = 800 × 1.0202013... = 816.161053387 TAO (interest 16.161053387)
...
(r = 3% × 0.5/0.8 = 1.875%/yr), after 30 days (720 h) Alice's debt is
`345 × (1 + 0.01875/8760)^720 = 345.532087 TUSDT`; HF drops 1.2 → 1.1982 and
available borrow is **0** — the CF-50% ceiling (345) is below her debt (345.53)
...
**Sub-hour clock.** Borrow at t=0: accrue at 30 min → nothing; at 60 min → exactly
1 hour charged (`last_update = 3_600_000`); at 61 min → still 1 hour (the 60 s
remainder carries); at 125 min → 2 hours (`last_update = 7_200_000`).
...
repay 0 → `ZeroAmount`; `debt_market = 2` → `InvalidDebtMarket`; seizure exceeding
the position → `CollateralAwardExceedsPosition`; repay 10,000 against a 100-rao debt
→ clamps to 100, clears, no error. (`CloseFactorExceeded` / `InvalidCollateralNetuid`
are declared but never returned — the close factor clamps instead of erroring.)
...
| Debt / interest | `floor(scaled × borrow_index / 1e18)`, scaled = `ceil(amount / index)`; interest = `debt − principal` |
